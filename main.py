...
def get_parser():
    ...
    parser.add_argument(
        "--adj-matrix",
        default="./graph/ucla_dual.yaml",
        help="path to the adjacency matrix file for line graph",
    )
...
class Processor:
    ...
    def load_model(self):
        ...
        if "lglt" in self.arg.model.lower():
            with open(self.arg.adj_matrix, "r") as f:
                adj_data = yaml.safe_load(f)
                adj_matrix = torch.zeros((19, 19))  # UCLA的边数是19
                for edge in adj_data["edges"]:
