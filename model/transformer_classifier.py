...
class TransformerTimeSeries(nn.Module):
    ...
    def forward(self, x, adj_mask=None):
        ...
        # print(f"嵌入后形状: {x.shape}")  # 应该是 [B*E, T, d_model]
        ...
        for i in range(B):
            ...
            # print(f"当前批次边形状: {curr_edges.shape}")  # 应该是 [E, T, d_model]

            # 对边级别进行时间序列处理
            edge_outputs = []
            for j in range(E):
                curr_edge_seq = curr_edges[j]  # [T, d_model]
                # print(f"当前边序列形状: {curr_edge_seq.shape}")  # 应该是 [T, d_model]

                # 添加CLS token (如果启用) - 修复此处的形状问题
                if self.use_cls_token:
                    # 修复：扩展维度确保形状匹配
                    cls_token_expanded = self.cls_token.expand(
                        1, -1, -1
                    )  # [1, 1, d_model]
                    # print(f"CLS token形状: {cls_token_expanded.shape}")
                    # print(
                    #     f"当前边序列形状(扩展): {curr_edge_seq.unsqueeze(0).shape}"
                    # )  # 应该是 [1, T, d_model]

                    # 确保形状匹配：先将序列扩展为3D
                    curr_edge_seq = curr_edge_seq.unsqueeze(0)  # [1, T, d_model]
                    curr_edge_seq = torch.cat(
                        [cls_token_expanded, curr_edge_seq], dim=1
                    )  # [1, 1+T, d_model]
                else:
                    curr_edge_seq = curr_edge_seq.unsqueeze(0)  # [1, T, d_model]

                # print(
                #     f"添加CLS后形状: {curr_edge_seq.shape}"
                # )  # 应该是 [1, T(+1), d_model]
            ...
            # print(f"合并边特征形状: {edge_outputs.shape}")  # 应该是 [E, d_model]
        ...
        # print(f"最终输出形状: {x.shape}")  # 应该是 [B, d_model]
